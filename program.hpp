 ...
 */
    ...
    bool judgeKey(std::string var);


    EvalState *statePoint;
    std::set<int> lineRef;
    std::unordered_map<int, std::string> lineSource;
    std::unordered_map<int, Statement *> linePoint;
    bool runAlready = false;
    int lineGoto = 0;//用于GOTO
    std::queue<std::string> inputStream;//用于INPUT
    bool stop = false;//用于END
};
